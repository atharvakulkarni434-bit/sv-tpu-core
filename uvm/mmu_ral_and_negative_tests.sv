...
class mmu_expected_error_catcher extends uvm_report_catcher;
    `uvm_object_utils(mmu_expected_error_catcher)

    // Counters to ensure the negative test actually triggered the expected failures
    int cov_errors_seen = 0;
    int sb_errors_seen  = 0;

    function new(string name = "mmu_expected_error_catcher");
        super.new(name);
    endfunction
    ...
    virtual function action_e catch();
        if (get_severity() == UVM_ERROR) begin
            ...
            // Check for Coverage Collector Errors
            if (get_id() == "MMU_COV") begin
                set_severity(UVM_INFO); // Demote to INFO
                cov_errors_seen++;
                return THROW;           // THROW continues processing with the new severity
            end
            
            // Check for Scoreboard Errors
            if (get_id() == "SB_DATA" || get_id() == "SB_REPORT") begin
                set_severity(UVM_INFO); // Demote to INFO
                sb_errors_seen++;
                return THROW;
            end
        ...
        // Let everything else pass through untouched
        return THROW;
    endfunction
endclass
...
class tc035a_force_skip_weight_load_test extends mmu_base_test;
    ...
    mmu_expected_error_catcher err_catcher;
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        // 1. Instantiate the catcher
        err_catcher = mmu_expected_error_catcher::type_id::create("err_catcher");
        
        // 2. Register it globally (null parent means it intercepts everything)
        uvm_report_cb::add(null, err_catcher);
    endfunction

    string dim_n_path = "tb_top.dut.u_mmu_controller.dim_n"; // Added path for dim_n
    ...
    virtual task main_phase(uvm_phase phase);
        ...
        `uvm_info(get_type_name(),
            $sformatf("forcing %s=1, %s=2, and %s=PE_CLEAR from IDLE - B1 (no_skip_weight_load) must fire",
                      start_path, dim_n_path, state_path), UVM_LOW)

        // 1. Force start=1 AND a legal dim_n to satisfy the SVA's updated antecedent
        ok  = uvm_hdl_force(start_path, 1'b1);
        ok &= uvm_hdl_force(dim_n_path, 3'd2); 
        if (!ok) `uvm_fatal(get_type_name(), "uvm_hdl_force failed")

        // 2. Wait for posedge so SVA samples: state==IDLE, start==1, dim_n==2 (Antecedent TRUE)
        @(posedge tb_top.clk);

        // 3. Force illegal state transition away from IDLE
        ok = uvm_hdl_force(state_path, mmu_force_state_pkg::PE_CLEAR);
        if (!ok) `uvm_fatal(get_type_name(), "uvm_hdl_force failed")

        // 4. Wait for posedge so SVA evaluates consequent: state==WEIGHT_LOAD (Consequent FALSE -> FIRE)
        @(posedge tb_top.clk);

        // 5. Release everything
        ok  = uvm_hdl_release(state_path);
        ok &= uvm_hdl_release(start_path);
        ok &= uvm_hdl_release(dim_n_path);
        if (!ok) `uvm_fatal(get_type_name(), "uvm_hdl_release failed")
    ...
    virtual function void report_phase(uvm_phase phase);
        ...
        // 3. Verify the negative test worked by checking the counters
        if (err_catcher.cov_errors_seen == 0 || err_catcher.sb_errors_seen == 0) begin
            `uvm_error(get_type_name(), "Test failed: Expected SVA/Scoreboard errors were not generated!")
        end else begin
            `uvm_info(get_type_name(), $sformatf("Negative test passed. Caught %0d COV errors and %0d SB errors.", 
                                                 err_catcher.cov_errors_seen, err_catcher.sb_errors_seen), UVM_NONE)
        end
    endfunction
...
class tc035b_force_pe_clear_hold_test extends mmu_base_test;
    ...
    virtual task force_extra_pe_clear_cycle();
        ...
        forever begin
            ...
            if (value == 1 && !seen) begin
                ...
                // REMOVED the extra @(posedge tb_top.clk) here. 
                // Force it IMMEDIATELY so it cannot naturally deassert on the next clock edge.
                void'(uvm_hdl_force(pe_clear_path, 1'b1));
                
                // Wait for the next clock edge where the violation (2nd continuous cycle) is observed
                @(posedge tb_top.clk);
                
                // Release and let RTL take over
    ...
    virtual function void report_phase(uvm_phase phase);
    super.report_phase(phase);
    if (err_catcher.sb_errors_seen == 0) begin
        `uvm_error(get_type_name(), "Test failed: Expected Scoreboard errors were not generated!")
    end else begin
        `uvm_info(get_type_name(), $sformatf("Negative test passed. Caught %0d SB errors.",
                                             err_catcher.sb_errors_seen), UVM_NONE)
    end
endfunction
