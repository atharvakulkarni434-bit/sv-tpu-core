...
measured, at the array's built physical size N = 4, for every legal `active_dim` (1..4). This superseded two earlier candidates: `2N` (an unverified early-design assumption) and `active_dim + N + 1` (a plausible-looking re-derivation from the RTL's structure that was never actually measured against silicon). The team ran the design for `active_dim` = 1 through 4 and confirmed the true, as-built latency is `active_dim + 5` in every case — this is now the sole contract, and `active_dim + N + 1` is retired.

| dim | 1 | 2 | 3 | 4 |
|---|---|---|---|---|
| Ratified latency (cycles) | 6 | 7 | 8 | 9 |

**Important — this formula is specific to the N = 4 build verified here, not parametric in `N`.** `active_dim + 5` was measured on this project's 4×4 array. If the design is ever elaborated at a different physical `N`, the "+5" constant is not guaranteed to hold — it reflects this build's pipeline depth (weight-load/PE-clear sequencing, array fill, and `deskew_capture`'s one-cycle capture lag), not a formula proven to scale with `N` in general. Re-measurement (or a first-principles re-derivation) is required before trusting `dim+5` at any other `N`.

**Why it isn't `2N`:** an earlier design phase assumed a horizontal-flow pipeline whose natural latency was 2N. Under DiP, output row `r` settles on the bottom accumulator at flow cycle `N + r`, and `deskew_capture`'s one-cycle capture lag means the last row is captured on the following cycle — a structurally tight window, not an off-by-one. Forcing a `2N` window would only coincide with correct sizing at `dim = N - 1`, which is unreachable for this build; for every legal `dim` (1..4) it either truncates the capture window or fails to match measured behavior. `2N` was never re-derivable from the DiP RTL — it was an artifact of the superseded architecture, and is now formally rejected (see `BUGS.md` Bug 7).

**Why it isn't `active_dim + N + 1` either:** that formula was a reasonable structural guess from reading the controller's counter sizing, but it was never checked against the RTL's measured behavior across `dim` = 1..3 (it only coincidentally matches `dim+5` at `dim = N = 4`). Direct measurement showed the real relationship does not track `N` the way that formula implied.

**Open follow-up (tracked separately, not blocking this lock):** `+5` is confirmed empirically, not yet derived from first principles against the pipeline's true minimum achievable depth. Whether that constant can be reduced through further optimization is a design question for a future pass, independent of what's asserted today.

Every artifact that depends on this number — the controller RTL comments, the controller SVA (B3), the signal-level performance checker, the UVM transaction-level latency checker (`mmu_cat6_tests.sv`), and the formal proof's notes — is reconciled against this single formula. There is no remaining disagreement between checkers.
...
| B1–B4 | `mmu_controller_sva.sv` | Controller FSM legality, including B3's result-latency property matching the ratified `active_dim + 5` formula. |
