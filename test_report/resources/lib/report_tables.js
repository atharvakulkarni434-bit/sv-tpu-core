...
function showNodeData(nodeData) {
    ...
    if (isVplan) {
        ...
        sourceData = nodeData.source_items;
    ...
    if (isVplan === true) {
        ...
        if (isStatementScoringEnabled != true && (statementInstanceData === undefined || statementInstanceData == null || statementInstanceData.length == 0)) {
        ...
        if (isStatementScoringEnabled != true && (statementData === undefined || statementData == null || statementData.length == 0) ){
            statementData = null;
            if (STATEMENT_TAB_INDEX >= 0) {
                disableTabs[disableTabs.length] = STATEMENT_TAB_INDEX;
            }		//$("#tabs").tabs({ disabled: [STATEMENT_TAB_INDEX] });
...
function getDataByType(type) {
    switch (type) {
        ...
        case SOURCE_TABLE:
            return sourceEntity == null ? emptyData : sourceEntity;
        case BLOCK_INSTANCE:
...
function diff(input) {
    ...
    //filter
    implementFilter($("input[name=search]"), $("button#btnResetSearch"), $("span#matches"), $("#treetable").fancytree("getTree"));
...
//======================================  Block ===================================================================
function diff_block_tab(input) {

    // Attach the fancytree widget to an existing <div id="tree"> element
    // and pass the tree options as an argument to the fancytree() function:
    diff_tab($("#block_table"), BLOCK, block_table_title_column_number, false, 0, blockSelectedAction);
    var block_table = $("#block_table").fancytree("getTree");
    //filter
    implementFilter($("input[name=block_search1]"), $("button#block_btnResetSearch1"), $("span#block_matches1"), block_table);

    showDiv('source_div', false);

}

function blockSelectedAction(event, data) {
    ...
    try {
        ...
        for (var i = 0 ; i < sourceData.length ; i++) {
            sourceIndex = "source_" + data.node.data.Index;
            sourceBlock = sourceData[i][sourceIndex];
            if (sourceBlock != null) {
                sourceEntity.push(sourceData[i]);
                hasSource = true;
                break;
            } else {
                hasSource = false;
        ...
        diff_tab($("#source_table"), SOURCE_TABLE, source_table_title_column_number, false, 0);
        var source_table = $("#source_table").fancytree("getTree");
        //filter
        implementFilter($("input[name=block_search2]"), $("button#block_btnResetSearch2"), $("span#block_matches2"), source_table);

    } catch (e) {

    }
    showDiv('source_div', hasSource);
...
function toggleSelectedAction(event, data) {
    ...
    if (data.node.data.subitems === undefined) {
        ...
        if (data.node.data.table_type === "reg") {
            ...
            if(data.node.data.table_type === "enum"){
                    try {
                ...
                try {
                    diff_tab($("#table_rnm_signal"), SIGNALS, signal_bit_rnm_title_column_number, false, 0);
                    var togle_rnm_table = $("#table_rnm_signal").fancytree("getTree");
                    //filter
                    implementFilter($("input[name=toggle_search4]"), $("button#toggle_btnResetSearch4"), $("span#toggle_matches4"), togle_rnm_table);
                    } catch (e) {
