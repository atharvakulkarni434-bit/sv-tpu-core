function showTestDataFromFile(fileName) {
    resetVsofOutput();
    ...
    var nodeData = null;
    start = fileMap[fileName].start;
    end = fileMap[fileName].end;

    reader.onloadend = function (evt) {
        if (evt.target.readyState == FileReader.DONE) { // DONE == 2
            try {
                nodeData = JSON.parse(evt.target.result);
            } catch (e) {
                nodeData = null;
            }
...
function showLoadingData(bShow) {
    ...
    document.getElementById('vsofOutput').style.visibility = bShow ? 'hidden' : 'visible';
    document.getElementById('filterVsofOutput_div').style.visibility = bShow ? 'hidden' : 'visible';
...
function hideAll() {
    ...
    document.getElementById('loading').innerHtml = "";
    document.getElementById('loading').style.height = 0;
    document.getElementById('test_tree_table').style.visibility = 'hidden';
    document.getElementById('filter_tree_div').style.visibility = 'hidden';
    document.getElementById('vsofOutput').style.visibility = 'hidden';
    document.getElementById('filterVsofOutput_div').style.visibility = 'hidden';
...
function showNodeData(nodeData) {
    ...
    vsofOutputData = nodeData.group_items;
    showVsofOutputTable();
...
function getDataByType(type) {
    switch (type) {
        case VSOF_OUTPUT:
            return vsofOutputData == null ? emptyData : vsofOutputData;
        case RUN:
...
function showVsofOutputTable() {
    resetVsofOutputData();
    load_table($("#vsofOutput"), VSOF_OUTPUT, vsofOutput_title_index, false, 0, loadVsofOutputData);
    var tree_table = $("#vsofOutput").fancytree("getTree");

    //filter
    implementFilter($("input[name=searchVsofOutput]"), $("button#btnResetSearchVsofOutput"), $("span#matchesVsofOutput"), tree_table);
}

function showRunsTable() {
    resetRunsData();
    runData = vsofOutputData[0].run_items;
...
function loadVsofOutputData(event, data) {
...
function resetVsofOutput() {
    resetVsofOutputData();
    load_table($("#vsofOutput"), -1, -1, false, 0);
}
    ...
    $("#test_tree_table").fancytree({
        ...
        activate: function (event, data) {
            if (selNode != null) {
                ...
                showTestDataFromFile(data.node.data.test_data);
